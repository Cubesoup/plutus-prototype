...
printType bs = runQuoteT $ prettyCfgText <$> (typecheckProgram 1000 True <=< annotateProgram <=< (liftEither . convertError . parseScoped)) bs
...
parseTypecheck gas = typecheckProgram gas True <=< annotateProgram <=< parseProgram
